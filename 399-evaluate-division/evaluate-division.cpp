...
public:
    ...
    vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
        ...
        for(int i=0; i<queries.size(); i++) {
            ...
            if(queries[i][0]==queries[i][1]&&m[queries[i][0]].size()==0)z=false;// edge case which I think should get corrected by Leetcode in near future
            if(z) {
                ans.push_back(p);
            } else {
                ans.push_back(-1.0);
